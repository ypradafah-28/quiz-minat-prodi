/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Kuis prediksi minat jurusan dalam 2 SESI:
   - Sesi 1: 5 soal UMUM -> scoring matrix 3 rumpun (12 fakultas) ->
             PASTI ada 1 rumpun pemenang (anti-draw).
   - Sesi 2: 5 soal MENGERUCUT di rumpun pemenang -> Top 5 prodi.
   - Bank soal: 132 soal (12 fakultas), tiap fakultas punya soal umum +
     soal bertema PENDIDIKAN agar prodi kependidikan lintas fakultas
     (mis. Pendidikan Ekonomi FEB, Pendidikan Matematika FMIPA) terangkat.
   ...
   2) MENAMBAH / MENGUBAH PRODI  →  edit array PRODI_LIST di bawah.
      ...
      TIPS: untuk mengangkat prodi kependidikan di fakultas mana pun,
      beri tag "pendidikan" pada soal bertema mengajar/guru.

   3) MENGUBAH JUMLAH SOAL / HASIL  →  edit objek KONFIGURASI di bawah
      (jumlahSoalPerSesi, jumlahProdiTampil).
   ========================================================================== */

/* ============ 1. KONFIGURASI UMUM ============ */
const KONFIGURASI = {
  jumlahSoalPerSesi: 5,   // berapa soal per sesi (Sesi 1 & Sesi 2)
  ...
  jumlahProdiTampil: 5,   // berapa prodi terbaik yang ditampilkan di akhir
...
/* ============ 2. RUMPUN PRODI ============ */
/* tags = kata kunci tiap rumpun (dipakai di scoring matrix Sesi 1) */
const RUMPUN = {
  r1: { nama: "Saintek, Kesehatan & Agro",
        tags: ["it", "data", "sains", "teknik", "kesehatan", "olahraga", "agro", "pangan"] },
  r2: { nama: "Soshum & Bisnis",
        tags: ["bisnis", "akuntansi", "hukum", "sosial", "hi", "komunikasi", "bahasa", "psikologi", "pendidikan"] },
  r3: { nama: "Seni & Budaya",
        tags: ["seni", "desain", "film", "musik", "budaya", "kuliner", "busana", "rias", "pariwisata"] },
};
...
const SOAL_BANK = [
  ...
  /* ---------- Soal bertema PENDIDIKAN di semua fakultas (id 121-132) ----------
     Tujuannya: saat minat/rumpun pendidikan terpilih, prodi kependidikan di
     fakultas mana pun (FEB, FMIPA, FBS, FT, FIKK, dll) punya peluang tinggi
     diangkat oleh scoring matrix. */
  {
    id: 121, kat: "FBS", rumpun: "r3", tags: "seni musik pendidikan",
    ringkas: "mengajar seni & bahasa (FBS)",
    teks: "Apakah kamu ingin mengajarkan seni tari, musik, atau rupa kepada murid di sekolah?",
    kataKunci: ["seni", "musik", "tari", "rupa", "menggambar", "melukis", "menyanyi", "gamelan", "budaya", "sastra", "bahasa", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum", "karya", "pentas", "vokal"],
  },
  {
    id: 122, kat: "FEB", rumpun: "r2", tags: "bisnis akuntansi pendidikan",
    ringkas: "pendidikan ekonomi & akuntansi (FEB)",
    teks: "Apakah kamu tertarik menjadi guru ekonomi atau akuntansi di SMK?",
    kataKunci: ["ekonomi", "akuntansi", "bisnis", "keuangan", "manajemen", "pajak", "perbankan", "wirausaha", "smk", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 123, kat: "FEB", rumpun: "r2", tags: "bisnis pendidikan",
    ringkas: "pendidikan bisnis & perkantoran (FEB)",
    teks: "Apakah kamu ingin mengajar administrasi perkantoran atau kewirausahaan di sekolah kejuruan?",
    kataKunci: ["administrasi", "perkantoran", "bisnis", "kewirausahaan", "manajemen", "keuangan", "smk", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 124, kat: "FH", rumpun: "r2", tags: "hukum pendidikan",
    ringkas: "pendidikan hukum & kewarganegaraan (FH)",
    teks: "Apakah kamu tertarik mengajar Pendidikan Pancasila atau menjadi dosen di bidang hukum?",
    kataKunci: ["hukum", "kewarganegaraan", "pancasila", "pengacara", "keadilan", "peraturan", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 125, kat: "FIKK", rumpun: "r1", tags: "olahraga pendidikan",
    ringkas: "pendidikan olahraga (FIKK)",
    teks: "Apakah kamu bercita-cita melatih atlet muda di sekolah atau menjadi guru olahraga?",
    kataKunci: ["olahraga", "atlet", "kebugaran", "kepelatihan", "jasmani", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum", "latihan", "prestasi"],
  },
  {
    id: 126, kat: "FISIPOL", rumpun: "r2", tags: "sosial pendidikan",
    ringkas: "pendidikan IPS & sejarah (FISIPOL)",
    teks: "Apakah kamu ingin menjadi guru sejarah, geografi, atau IPS di sekolah?",
    kataKunci: ["sejarah", "geografi", "ips", "sosial", "masyarakat", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 127, kat: "FK", rumpun: "r1", tags: "kesehatan pendidikan",
    ringkas: "pendidikan kesehatan (FK)",
    teks: "Apakah kamu tertarik menjadi dosen atau pengajar di bidang kedokteran dan kesehatan?",
    kataKunci: ["kedokteran", "kesehatan", "dokter", "keperawatan", "medis", "rumah sakit", "dosen", "pendidikan", "mengajar", "kampus", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 128, kat: "FKP", rumpun: "r1", tags: "agro pangan pendidikan",
    ringkas: "pendidikan agro & pangan (FKP)",
    teks: "Apakah kamu ingin menjadi penyuluh atau pengajar di bidang pangan dan pertanian?",
    kataKunci: ["pertanian", "pangan", "agribisnis", "penyuluhan", "teknologi pangan", "tanaman", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 129, kat: "FMIPA", rumpun: "r1", tags: "sains pendidikan",
    ringkas: "pendidikan sains (FMIPA)",
    teks: "Apakah kamu ingin mengajar fisika, kimia, atau biologi di sekolah?",
    kataKunci: ["fisika", "kimia", "biologi", "matematika", "sains", "laboratorium", "eksperimen", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 130, kat: "FPSI", rumpun: "r2", tags: "psikologi pendidikan",
    ringkas: "pendidikan psikologi & konseling (FPSI)",
    teks: "Apakah kamu ingin menjadi konselor sekolah atau dosen psikologi?",
    kataKunci: ["psikologi", "konselor", "bimbingan", "konseling", "perilaku", "emosi", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 131, kat: "FT", rumpun: "r1", tags: "teknik pendidikan",
    ringkas: "pendidikan teknik & vokasi (FT)",
    teks: "Apakah kamu tertarik mengajar di SMK bidang teknik, listrik, atau mesin?",
    kataKunci: ["teknik", "listrik", "mesin", "bangunan", "elektro", "otomotif", "smk", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
  {
    id: 132, kat: "FV", rumpun: "r1", tags: "it pendidikan",
    ringkas: "pendidikan vokasi digital (FV)",
    teks: "Apakah kamu ingin mengembangkan media pembelajaran atau mengajar keterampilan digital?",
    kataKunci: ["media pembelajaran", "multimedia", "digital", "teknologi", "edukasi", "aplikasi", "video", "guru", "pendidikan", "mengajar", "sekolah", "siswa", "murid", "dosen", "pembelajaran", "pengajar", "kurikulum"],
  },
];
...
/* ============ 5. STATE APLIKASI ============ */
let soalAktif = [];          // soal yang terpilih (sesi 1 + sesi 2)
let jawaban = [];            // skor jawaban untuk tiap soal (urutan sama dengan soalAktif)
let indexSekarang = 0;       // posisi soal dalam sesi yang sedang berjalan (0..4)
...
let sesiAktif = "sesi1";     // "sesi1" | "sesi2"
let skorFakultas = {};       // skor tiap fakultas -> dasar pemilihan soal sesi 2
let rumpunPemenang = null;   // kode rumpun pemenang hasil sesi 1 ("r1"/"r2"/"r3")
let rumpunPeringkat = [];    // [r1,r2,r3] diurutkan (peringkat unik, tanpa draw)
let skorRumpunDetail = [];   // detail skor tiap rumpun untuk ditampilkan
...
const el = {
  ...
  sectionRumpun: document.getElementById("section-rumpun"),
  sectionHasil:  document.getElementById("section-result"),
  sesiBadge:     document.getElementById("sesi-badge"),
  ...
  analisJudul:   document.getElementById("analis-judul"),
  rumpunList:    document.getElementById("rumpun-list"),
  ...
  btnSesi2:      document.getElementById("btn-sesi2"),
...
// Peta kode fakultas -> rumpun (diturunkan dari SOAL_BANK, konsisten per fakultas)
const rumpunPerFakultas = {};
SOAL_BANK.forEach((s) => {
  if (!rumpunPerFakultas[s.kat]) rumpunPerFakultas[s.kat] = s.rumpun;
});

// Deskripsi singkat tiap rumpun (untuk layar hasil sesi 1)
const RUMPUN_DESKRIPSI = {
  r1: "Teknologi, sains, kesehatan, olahraga, pangan, dan keterampilan terapan. Cocok untuk kamu yang suka eksperimen, logika, dan praktik.",
  r2: "Ekonomi, bisnis, hukum, sosial, komunikasi, psikologi, dan pendidikan. Cocok untuk kamu yang suka berinteraksi, menganalisis, dan membantu orang lain.",
  r3: "Seni, desain, film, musik, bahasa, dan budaya. Cocok untuk kamu yang kreatif, ekspresif, dan peka terhadap keindahan.",
};

// Contoh fakultas tiap rumpun (untuk layar hasil sesi 1)
const RUMPUN_FAKULTAS = {
  r1: ["FMIPA", "FT", "FK", "FIKK", "FKP", "FV"],
  r2: ["FEB", "FISIPOL", "FIP", "FH", "FPSI"],
  r3: ["FBS"],
};

// =====================================================================
// SESI 1 : soal UMUM, menyebar luas (tiap soal dari fakultas berbeda).
// Pola rumpun: r1, r2, r3, lalu r1/r2 (rumpun dengan akumulasi skor terdepan duluan).
// =====================================================================
function pilihSoalSesi1(idx) {
  let rumpunTarget;
  if (idx < 3) {
    rumpunTarget = ["r1", "r2", "r3"][idx];
  } else {
    const tot = { r1: 0, r2: 0 };
    jawaban.forEach((a, i) => {
      if (a && soalAktif[i]) {
        const r = rumpunPerFakultas[soalAktif[i].kat];
        if (r === "r1" || r === "r2") tot[r] += a;
      }
    });
    const urut = ["r1", "r2"].sort((x, y) => tot[y] - tot[x]);
    rumpunTarget = idx === 4 ? urut[1] : urut[0];
  }

  const fakTerpakai = new Set(soalAktif.map((s) => s.kat));
  const kodeFak = Object.keys(FAKULTAS_NAMA)
    .filter((f) => rumpunPerFakultas[f] === rumpunTarget && !fakTerpakai.has(f));

  let pool = [];
  kodeFak.forEach((k) => {
    pool = pool.concat(SOAL_BANK.filter((s) => s.kat === k && !idTerpakai.has(s.id)));
  });
  if (!pool.length) pool = SOAL_BANK.filter((s) => s.rumpun === rumpunTarget && !idTerpakai.has(s.id));
  if (!pool.length) pool = SOAL_BANK.filter((s) => !idTerpakai.has(s.id));
  return pilihAcak(pool);
}

// =====================================================================
// SESI 2 : soal MENGERUCUT hanya dari rumpun pemenang sesi 1.
// Prioritas fakultas dengan skor sesi 1 tertinggi, pola 2-2-1.
// =====================================================================
function pilihSoalSesi2(idx) {
  const fakRumpun = Object.keys(FAKULTAS_NAMA)
    .filter((f) => rumpunPerFakultas[f] === rumpunPemenang)
    .sort((a, b) => (skorFakultas[b] || 0) - (skorFakultas[a] || 0));

  const pola = [0, 1, 2, 0, 1]; // distribusi 2-2-1
  const kode = fakRumpun[pola[idx] % fakRumpun.length];

  let pool = SOAL_BANK.filter((s) => s.kat === kode && !idTerpakai.has(s.id));
  if (!pool.length) pool = SOAL_BANK.filter((s) => s.rumpun === rumpunPemenang && !idTerpakai.has(s.id));
  if (!pool.length) pool = SOAL_BANK.filter((s) => !idTerpakai.has(s.id));
  return pilihAcak(pool);
}

function pilihSoalBerikutnya(idx) {
  return sesiAktif === "sesi1" ? pilihSoalSesi1(idx) : pilihSoalSesi2(idx);
}

// Menampilkan satu "layar" (section) dan menyembunyikan yang lain
function tampilkanSection(target) {
  [el.sectionStart, el.sectionQuiz, el.sectionAnalis, el.sectionRumpun, el.sectionHasil].forEach((s) => {
...
/* ============ 8. ALUR KUIS ============ */
// Memulai / mengulang kuis (mulai dari Sesi 1)
function mulaiKuis() {
  ...
  // Inisialisasi skor tiap fakultas (dipakai untuk urutan soal sesi 2)
  skorFakultas = {};
  Object.keys(FAKULTAS_NAMA).forEach((f) => { skorFakultas[f] = 0; });

  // Soal pertama sesi 1: sampling luas
...
// Menampilkan soal pada indeks tertentu (dalam sesi saat ini)
function renderSoal(idx) {
  // Soal yang sedang aktif selalu elemen terakhir soalAktif (idx = posisi dalam sesi)
  const soal = soalAktif[soalAktif.length - 1];
  const total = KONFIGURASI.jumlahSoalPerSesi;

  // Teks soal, badge sesi, dan progres
  el.sesiBadge.textContent = sesiAktif === "sesi1" ? "Peminatan" : "Minat Prodi";
...
function pilihOpsi(skor, btn) {
  ...
  // Simpan jawaban; posisi mengikuti soalAktif GLOBAL (indexSekarang
  // direset ke 0 saat sesi 2 dimulai).
  const soal = soalAktif[soalAktif.length - 1];
  jawaban[soalAktif.length - 1] = skor;

  // Perbarui skor tiap fakultas -> dasar urutan soal sesi 2
  ...
  setTimeout(() => {
    if (indexSekarang + 1 < KONFIGURASI.jumlahSoalPerSesi) {
      indexSekarang++;
      ...
      tampilkanAnalisis(); // sesi selesai
    }
  }, 350);
}

// Layar "menganalisis" lalu tampilkan hasil sesuai sesi
function tampilkanAnalisis() {
  const sesi1 = sesiAktif === "sesi1";
  // Judul layar analisis menyesuaikan sesi (subtitle "menghitung scoring" dihapus)
  if (el.analisJudul) {
    el.analisJudul.innerHTML = sesi1
      ? 'Menentukan rumpun minatmu<span class="dot-anim"><span>.</span><span>.</span><span>.</span></span>'
      : 'Rekomendasi prodi<span class="dot-anim"><span>.</span><span>.</span><span>.</span></span>';
  }
  tampilkanSection(el.sectionAnalis);

  setTimeout(() => {
    if (sesi1) {
      const hasilRumpun = hitungRumpunSesi1();
      renderRumpun(hasilRumpun);
    } else {
...
// Mulai sesi 2 (melanjutkan dari rumpun pemenang sesi 1)
function mulaiSesi2() {
  if (!rumpunPemenang) return; // aman: belum ada pemenang rumpun
  sesiAktif = "sesi2";
  indexSekarang = 0;
  kunciJawaban = false;

  const s = pilihSoalBerikutnya(0);
  soalAktif.push(s);
  idTerpakai.add(s.id);
...
/* =====================================================================
   SESI 1 — SCORING MATRIX RUMPUN
   Setiap soal memberi skor ke 3 rumpun:
     bobot = +3 (fakultas soal ada di rumpun itu)
           + jumlah tag soal yang termasuk tag rumpun (matrix kata kunci)
   Antidraw: peringkat diurutkan dengan tie-break berlapis:
     1) skor rumpun  2) jawaban positif langsung  3) jawaban positif serumpun
     lalu kelompok yang masih seri diacak internal -> PASTI ada 1 pemenang.
   ===================================================================== */
function hitungRumpunSesi1() {
  const skor = { r1: 0, r2: 0, r3: 0 };
  const pos = { r1: 0, r2: 0, r3: 0 };      // akumulasi jawaban positif pada soal langsung rumpun
  const posTag = { r1: 0, r2: 0, r3: 0 };   // akumulasi jawaban positif pada soal serumpun (tag)

  for (let i = 0; i < KONFIGURASI.jumlahSoalPerSesi; i++) {
    const q = soalAktif[i];
    const a = jawaban[i] || 0;
    if (!q || !a) continue;

    Object.keys(skor).forEach((R) => {
      let bobot = 0;
      if (rumpunPerFakultas[q.kat] === R) bobot += 3; // soal langsung dari rumpun itu
      const tagCocok = q.tags.split(" ").filter((t) => RUMPUN[R].tags.includes(t)).length;
      bobot += tagCocok; // overlap tag = matrix kata kunci

      skor[R] += a * bobot;
      if (a > 0) {
        if (rumpunPerFakultas[q.kat] === R) pos[R] += a;
        if (tagCocok > 0) posTag[R] += a;
      }
    });
  }

  // Susun peringkat dengan tie-break berlapis (1 pemenang dipastikan)
  const daftar = Object.keys(skor).map((R) => ({
    kode: R,
    skor: skor[R],
    pos: pos[R],
    posTag: posTag[R],
  }));
  daftar.sort((a, b) => b.skor - a.skor || b.pos - a.pos || b.posTag - a.posTag);

  // Anti-draw final: kelompok yang masih seri penuh diacak internal
  let i = 0;
  while (i < daftar.length) {
    let j = i;
    while (
      j + 1 < daftar.length &&
      daftar[j + 1].skor === daftar[i].skor &&
      daftar[j + 1].pos === daftar[i].pos &&
      daftar[j + 1].posTag === daftar[i].posTag
    ) j++;
    if (j > i) {
      const sub = acakArray(daftar.slice(i, j + 1));
      daftar.splice(i, j - i + 1, ...sub);
    }
    i = j + 1;
  }

  rumpunPeringkat = daftar.map((d) => d.kode);
  rumpunPemenang = rumpunPeringkat[0];
  skorRumpunDetail = daftar;

  return { peringkat: daftar, pemenang: rumpunPemenang };
}

// Layar hasil sesi 1: 3 rumpun + juara (unik) + minat di dalam rumpun pemenang
function renderRumpun(hasil) {
  const list = hasil.peringkat;
  const min = list[list.length - 1].skor;
  const max = list[0].skor;
  const rentang = max - min;

  el.rumpunList.innerHTML = "";
  list.forEach((item, i) => {
    const r = RUMPUN[item.kode];
    const juara = item.kode === hasil.pemenang;
    const persen = rentang > 0 ? Math.round(((item.skor - min) / rentang) * 100) : 50;

    const card = document.createElement("div");
    card.className = juara ? "rumpun-card rumpun-juara anim-masuk" : "rumpun-card anim-masuk";
    card.style.animationDelay = `${i * 130}ms`;

    card.innerHTML = `
      <div class="rumpun-head">
        <div class="rank-badge rank-${i + 1}">${i + 1}</div>
        <div class="rumpun-info">
          <p class="rumpun-nama">${esc(r.nama)}</p>
        </div>
        <div class="rumpun-pct">
          <span class="pct-angka">${persen}%</span>
          <div class="pct-track"><div class="pct-fill" style="width:${persen}%"></div></div>
        </div>
      </div>
      <p class="rumpun-desk">${esc(RUMPUN_DESKRIPSI[item.kode])}</p>
      ${juara ? `<div class="rumpun-fak">
        <span class="detail-label">Fakultas di dalam rumpun ini</span>
        <div class="minat-contoh">${RUMPUN_FAKULTAS[item.kode].map((f) =>
          `<span class="chip chip-${item.kode}">${esc(f)} - ${esc(FAKULTAS_NAMA[f].replace("Fakultas ", ""))}</span>`).join("")}
        </div>
      </div>` : ""}
    `;

    el.rumpunList.appendChild(card);
  });

  tampilkanSection(el.sectionRumpun);
}
...
function renderHasil(skorProdi) {
  ...
  const top5 = [...skorProdi].sort((a, b) => b.skor - a.skor).slice(0, KONFIGURASI.jumlahProdiTampil);
  ...
  top5.forEach((p, i) => {
    ...
    card.innerHTML = `
      <div class="result-head" role="button" tabindex="0" aria-expanded="false" aria-label="Buka detail ${esc(p.nama)}">
        ...
        <span class="lihat-selengkapnya">lihat selengkapnya</span>
      </div>
...
el.btnSesi2.addEventListener("click", mulaiSesi2);
